...
class FakeVectorStoreClient:
    def __init__(self):
        ...
        self.reject_existing_file = False
    ...
    def attach(self, vector_store_id, **kwargs):
        ...
        if self.reject_existing_file:
            raise RuntimeError("file cannot be attached to multiple vector stores")
        return SimpleNamespace(id="vsf_candidate", status="in_progress")

    def retrieve(self, file_id, **kwargs):
        self.calls.append(("retrieve", (file_id,), kwargs))
        return SimpleNamespace(id="vsf_candidate", status="completed")

    def create_batch(self, vector_store_id, **kwargs):
        self.calls.append(("create_batch", (vector_store_id,), kwargs))
        return SimpleNamespace(id="vsfb_candidate", status="in_progress", file_counts={"completed": 1})

    def retrieve_batch(self, batch_id, **kwargs):
        self.calls.append(("retrieve_batch", (batch_id,), kwargs))
        return SimpleNamespace(id="vsfb_candidate", status="completed", file_counts={"completed": 2})
...
def test_existing_file_rejection_reports_the_documented_candidate_fallback_boundary():
    client = FakeVectorStoreClient()
    client.reject_existing_file = True
